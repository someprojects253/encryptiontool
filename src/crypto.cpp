...
void Crypto::run()
{
    ...
    if(encryptToggle == "Decrypt") {
        ...
        if(isOuter) {
            ...
            inputFileHandle.seekg(iv.size(), std::ios::beg);
            ciphertext_size = filesize - iv.size();
        }
    }
        ...
        if(!(mode == "SIV" || mode == "CCM(16,4)" || ciphertext_size < (1 << 20))){
            for(size_t i = 0; i < numchunks; i++){
                ...
                if(isAEAD) {
                    ...
                    if(isOuter && encryptToggle == "Decrypt") hmac->update(buffer);
                    enc->update(buffer);
                    if(isOuter && encryptToggle == "Encrypt") hmac->update(buffer);
                }
